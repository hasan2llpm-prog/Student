...
async function openGifts(){const body=page('student-profile-gifts-page','هدايا الحسابات');body.innerHTML=`<div class="srt-card"><div class="srt-field"><label>ابحث عن المستخدم</label><input id="srt-user-search" placeholder="الاسم أو اسم المستخدم"></div><div id="srt-user-results" class="srt-search-results"></div></div><div class="srt-card"><div id="srt-selected" class="srt-status">لم يتم اختيار مستخدم.</div><div class="srt-field"><label>نوع الهدية</label><select id="srt-kind"><option value="verification">علامة توثيق</option><option value="custom_badge">علامة مخصصة</option><option value="frame">إطار للحساب</option></select></div><div class="srt-field"><label id="srt-value-label">لون التوثيق</label><input id="srt-value" value="blue" placeholder="blue / red / orange"></div><div class="srt-field"><label>اسم الهدية / الوصف</label><input id="srt-label" maxlength="60" placeholder="مثال: هدية الإدارة"></div><div class="srt-field"><label>لون العلامة المخصصة (اختياري)</label><input id="srt-color" maxlength="7" placeholder="#8b5cf6"></div><div class="srt-actions"><button class="srt-btn" id="srt-grant" disabled><i class="fa-solid fa-gift"></i> منح الهدية</button></div><div id="srt-gift-status" style="margin-top:10px"></div></div>`;let selected=null,seq=0,timer;const search=body.querySelector('#srt-user-search'),results=body.querySelector('#srt-user-results'),selectedBox=body.querySelector('#srt-selected'),grant=body.querySelector('#srt-grant'),kind=body.querySelector('#srt-kind'),value=body.querySelector('#srt-value'),valueLabel=body.querySelector('#srt-value-label');function updateKind(){if(kind.value==='verification'){valueLabel.textContent='لون التوثيق';value.placeholder='blue / red / orange';if(!['blue','red','orange'].includes(value.value))value.value='blue'}else if(kind.value==='custom_badge'){valueLabel.textContent='رمز العلامة';value.placeholder='مثال: ★ أو 🎓';value.value='★'}else{valueLabel.textContent='نوع الإطار المتحرك';value.placeholder='fire / blue-fire / neon / lightning / royal / ember';value.value='fire'}}kind.onchange=updateKind;search.oninput=()=>{clearTimeout(timer);timer=setTimeout(async()=>{const q=String(search.value||'').trim().replace(/[,%()]/g,' ');if(q.length<2){results.innerHTML='';return}const my=++seq;const{data,error}=await client.from('profiles').select('id,full_name,username,email,is_verified,verification_color').or(`full_name.ilike.%${q}%,username.ilike.%${q}%`).limit(20);if(my!==seq)return;if(error){results.innerHTML=`<div class="srt-status warn">${esc(error.message)}</div>`;return}results.innerHTML=(data||[]).map(u=>`<div class="srt-result" data-id="${esc(u.id)}"><div><strong>${esc(u.full_name||u.username||'مستخدم')}${badge(u)}</strong><div class="srt-meta">@${esc(u.username||'')} · ${esc(u.email||'')}</div></div></div>`).join('');results.querySelectorAll('[data-id]').forEach((el,i)=>el.onclick=()=>{selected=(data||[])[i];results.querySelectorAll('.srt-result').forEach(x=>x.classList.toggle('active',x===el));selectedBox.innerHTML=`المستخدم المختار: <strong>${esc(selected.full_name||selected.username)}</strong>`;grant.disabled=false})},260)};grant.onclick=()=>{if(!selected)return;const giftKind=kind.value,giftValue=value.value.trim(),label=body.querySelector('#srt-label').value.trim(),color=body.querySelector('#srt-color').value.trim();confirmAction('تأكيد منح الهدية',`سيتم منح ${selected.full_name||selected.username} هذه الهدية وسيصله إشعار من مدير التطبيق.`,async()=>{grant.disabled=true;const rpcResult=giftKind==='frame'?await client.rpc('student_admin_grant_profile_frame',{p_user:selected.id,p_frame_key:giftValue||'fire',p_days:null}):await client.rpc('student_admin_gift_profile_cosmetic',{p_user:selected.id,p_kind:giftKind,p_value:giftValue||null,p_label:label||null,p_color:color||null});const error=rpcResult.error;grant.disabled=false;if(error)throw error;body.querySelector('#srt-gift-status').innerHTML='<div class="srt-status ok">تم منح الهدية وإرسال الإشعار بنجاح.</div>'})}}
