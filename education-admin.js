...
(function () {
    ...
    function profileBadge(profile, size = 13) {
        return typeof window.studentVerificationBadge === "function"
            ? window.studentVerificationBadge(profile || {}, size)
            : "";
    }

    function framedAvatar(profile, fallback = "👨‍🏫", size = 42) {
        const inner = profile?.avatar_url
            ? `<img src="${esc(profile.avatar_url)}" alt="" loading="lazy" decoding="async" style="width:${size}px;height:${size}px;border-radius:50%;object-fit:cover">`
            : `<span style="width:${size}px;height:${size}px;border-radius:50%;display:grid;place-items:center;background:#eef3f8">${fallback}</span>`;
        return typeof window.studentProfileFrameWrap === "function"
            ? window.studentProfileFrameWrap(profile || {}, inner)
            : inner;
    }
    ...
    async function profileMap(userIds) {
        ...
        const { data, error } = await database.from("profiles")
            .select("id,full_name,username,avatar_url,bio,is_verified,verification_color,custom_badge_icon,custom_badge_label,custom_badge_color,profile_frame_url")
            .in("id", unique);
        if (error) throw error;
    ...
    async function openSubject(context){
        ...
        const isUniversity=context.education_type==="university";
        let tq=isUniversity
            ? d.from("teacher_university_assignments").select("id,teacher_id,subject_id,university_level_id").eq("subject_id",context.subject_id)
            : d.from("teacher_school_assignments").select("id,teacher_id,subject_id,level_id,track_id").eq("subject_id",context.subject_id);
        if(isUniversity)tq=tq.eq("university_level_id",context.university_level_id);else{tq=tq.eq("level_id",context.level_id);tq=context.track_id?tq.eq("track_id",context.track_id):tq.is("track_id",null);}
        ...
        const teachers=assign.length?`<div class="ste-name" style="margin:15px 0 8px">مدرسو المادة</div>${assign.map(a=>{const p=profiles.get(a.teacher_id)||{};return `<button type="button" class="ste-card ste-row" data-assignment="${esc(a.id)}" style="width:100%;text-align:right;cursor:pointer"><span class="ste-avatar" style="overflow:visible">${framedAvatar(p,"👨‍🏫",42)}</span><span class="ste-grow"><span class="ste-name">${esc(p.full_name||p.username||"مدرس")}${profileBadge(p,13)}</span><span class="ste-muted" style="display:block">${esc(p.bio||"عرض محتوى المدرس")}</span></span><span>‹</span></button>`}).join("")}`:"";
    ...
    async function openTeacherMaterials(assignment, context, profiles) {
        ...
        try {
            ...
            const html = `<div class="ste-card ste-row"><span class="ste-avatar" style="overflow:visible">${framedAvatar(p,"👨‍🏫",42)}</span><span class="ste-grow"><span class="ste-name">${esc(p.full_name || p.username || "مدرس")}${profileBadge(p,13)}</span><span class="ste-muted" style="display:block">${esc(p.bio || "")}</span></span></div>` +
    ...
    function renderTeacherRegistration(user) {
        ...
        overlay.querySelector("#ste-submit-request").onclick = async function () {
            ...
            if (!specialization) { const input=overlay.querySelector("#ste-specialization"); input?.focus(); input?.setAttribute("placeholder","اكتب التخصص أولًا"); return; }
            this.disabled = true;
            try {
    ...
    async function renderApprovedTeacher(user, teacher) {
        try {
            ...
            if (save) save.onclick = async function () {
                ...
                if (!title) { overlay.querySelector("#ste-material-title")?.focus(); return; }
                if (materialType !== "text" && !url) { overlay.querySelector("#ste-material-url")?.focus(); return; }
                this.disabled = true;
                try {
    ...
    async function openAdmin(passedClient, options = {}) {
        ...
        try {
            ...
            const html = rows.length ? rows.map((r) => {
                ...
                return `<div class="ste-card" data-request="${esc(r.id)}"><div class="ste-row"><span class="ste-avatar" style="overflow:visible">${framedAvatar(p,"👨‍🏫",42)}</span><span class="ste-grow"><div class="ste-name">${esc(p.full_name || p.username || "مدرس")}${profileBadge(p,13)}</div><div class="ste-muted">${esc(r.specialization)} — خبرة ${esc(r.experience_years)} سنة</div></span></div><div class="ste-muted" style="margin-top:9px">${esc(r.qualifications || "لا توجد مؤهلات مكتوبة")}</div>${r.document_url ? `<a href="${esc(r.document_url)}" target="_blank" rel="noopener" class="ste-badge">فتح الوثيقة</a>` : ""}<div class="ste-actions"><button class="ste-btn" data-approve>قبول</button><button class="ste-btn danger" data-reject>رفض</button></div></div>`;
...
/* =========================================================
   Student Admin - Final Release Tools
   Recent users + profile gifts + education structure audit
========================================================= */
(function(){
'use strict';
if(window.__studentFinalReleaseTools) return;
window.__studentFinalReleaseTools=true;
let client=null,adminUser=null;
const esc=v=>String(v??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[ch]));
function styles(){if(document.getElementById('student-release-tools-style'))return;const s=document.createElement('style');s.id='student-release-tools-style';s.textContent=`
.srt-page{position:fixed;inset:0;z-index:10095;background:#f5f7fb;display:none;flex-direction:column;direction:rtl;color:#172033}.srt-page.show{display:flex}.srt-head{height:68px;background:#fff;border-bottom:1px solid #e6eaf0;display:flex;align-items:center;gap:10px;padding:0 14px;flex:0 0 auto}.srt-back,.srt-btn{border:0;cursor:pointer;font:inherit}.srt-back{width:42px;height:42px;border-radius:14px;background:#eef2f6}.srt-title{font-size:19px;font-weight:900;flex:1}.srt-body{width:min(900px,100%);margin:auto;flex:1;overflow:auto;padding:14px;box-sizing:border-box}.srt-card{background:#fff;border:1px solid #e7ebf0;border-radius:18px;padding:14px;margin-bottom:10px;box-shadow:0 4px 15px rgba(15,23,42,.03)}.srt-user{display:flex;align-items:center;gap:10px}.srt-user-main{min-width:0;flex:1}.srt-name{font-weight:900}.srt-meta{font-size:12px;color:#748092;margin-top:4px;overflow-wrap:anywhere}.srt-time{font-size:10px;color:#99a2ae}.srt-field{display:grid;gap:6px;margin-bottom:12px}.srt-field label{font-weight:800;font-size:13px}.srt-field input,.srt-field select{width:100%;box-sizing:border-box;border:1px solid #d9e0e8;border-radius:12px;padding:11px;background:#fff;font:inherit}.srt-btn{background:#087cff;color:#fff;border-radius:12px;padding:10px 14px;font-weight:800}.srt-btn.secondary{background:#eef2f6;color:#263442}.srt-btn.danger{background:#e83d50}.srt-actions{display:flex;gap:8px;flex-wrap:wrap}.srt-status{padding:11px 13px;border-radius:13px;background:#eef5ff;color:#36506f;margin-bottom:10px;font-size:12px;line-height:1.7}.srt-status.ok{background:#edf9f1;color:#176b38}.srt-status.warn{background:#fff7e8;color:#8a5a00}.srt-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:9px}.srt-metric{background:#fff;border:1px solid #e7ebf0;border-radius:15px;padding:13px}.srt-metric strong{display:block;font-size:22px}.srt-metric span{font-size:11px;color:#788393}.srt-search-results{display:grid;gap:7px;max-height:320px;overflow:auto}.srt-result{display:flex;align-items:center;gap:9px;border:1px solid #e7ebf0;border-radius:13px;padding:10px;cursor:pointer;background:#fff}.srt-result.active{border-color:#087cff;background:#f2f8ff}.srt-empty{text-align:center;color:#7d8795;padding:50px 15px}.srt-confirm{position:fixed;inset:0;z-index:10110;background:rgba(8,17,30,.5);display:flex;align-items:flex-end;justify-content:center;padding:12px}.srt-confirm-card{width:min(520px,100%);background:#fff;border-radius:22px;padding:18px}
...
async function openRecent(){const body=page('student-recent-users-page','المسجلون حديثًا');body.innerHTML='<div class="srt-empty">جارٍ تحميل أحدث الحسابات...</div>';let query=client.from('profiles').select('id,full_name,username,email,role,created_at,avatar_url,is_verified,verification_color,profile_frame_url').order('created_at',{ascending:false}).limit(30);let{data,error}=await query;if(error){const rpc=await client.rpc('student_admin_recent_users',{p_limit:30});data=rpc.data;error=rpc.error}if(error){body.innerHTML=`<div class="srt-status warn">${esc(error.message)}</div>`;return}const rows=data||[];body.innerHTML=`<div class="srt-status">آخر ${rows.length} حسابًا مرتبة من الأحدث إلى الأقدم.</div>${rows.map(u=>`<article class="srt-card srt-user">${u.avatar_url && typeof window.studentProfileFrameWrap==='function'?window.studentProfileFrameWrap(u,`<img src="${esc(u.avatar_url)}" alt="" style="width:44px;height:44px;border-radius:50%;object-fit:cover;display:block">`):''}<div class="srt-user-main"><div class="srt-name">${esc(u.full_name||u.username||'مستخدم')}${badge(u)}</div><div class="srt-meta">${esc(u.email||'لا يوجد بريد ظاهر')}</div><div class="srt-meta">@${esc(u.username||'')} · ${esc(u.role||'student')}</div></div><time class="srt-time">${u.created_at?esc(new Date(u.created_at).toLocaleString('ar-IQ')):''}</time></article>`).join('')||'<div class="srt-empty">لا توجد حسابات.</div>'}`}

async function openGifts(){
const body=page('student-profile-gifts-page','هدايا الحسابات');
body.innerHTML=`<div class="srt-card"><div class="srt-actions" style="margin-bottom:12px"><button class="srt-btn" id="srt-gift-self"><i class="fa-solid fa-user-shield"></i> إهداء لنفسي</button></div><div class="srt-field"><label>ابحث عن المستخدم</label><input id="srt-user-search" placeholder="الاسم أو اسم المستخدم"></div><div id="srt-user-results" class="srt-search-results"></div></div><div class="srt-card"><div id="srt-selected" class="srt-status">لم يتم اختيار مستخدم.</div><div class="srt-field"><label>نوع الهدية</label><select id="srt-kind"><option value="verification">علامة توثيق</option><option value="custom_badge">علامة مخصصة</option><option value="frame">إطار للحساب</option></select></div><div id="srt-normal-value" class="srt-field"><label id="srt-value-label">لون التوثيق</label><input id="srt-value" value="blue"></div><div id="srt-frame-fields" style="display:none"><div class="srt-field"><label>الإطار</label><select id="srt-frame-key"><option value="fire">🔥 نار مشتعلة</option><option value="blue-fire">💙 نار زرقاء</option><option value="neon">🌈 نيون</option><option value="lightning">⚡ برق</option><option value="royal">👑 ملكي ذهبي</option><option value="ember">🔥 جمر</option></select></div><div class="srt-field"><label>المدة بالأيام</label><input id="srt-frame-days" type="number" min="1" placeholder="فارغ = دائم"></div></div><div class="srt-field"><label>اسم الهدية / الوصف</label><input id="srt-label" maxlength="60"></div><div class="srt-field"><label>لون العلامة المخصصة (اختياري)</label><input id="srt-color" maxlength="7" placeholder="#8b5cf6"></div><div class="srt-actions"><button class="srt-btn" id="srt-grant" disabled><i class="fa-solid fa-gift"></i> منح الهدية</button></div><div id="srt-gift-status" style="margin-top:10px"></div></div>`;
let selected=null,seq=0,timer;
const search=body.querySelector('#srt-user-search'),results=body.querySelector('#srt-user-results'),selectedBox=body.querySelector('#srt-selected'),grant=body.querySelector('#srt-grant'),kind=body.querySelector('#srt-kind'),value=body.querySelector('#srt-value'),valueLabel=body.querySelector('#srt-value-label'),frameFields=body.querySelector('#srt-frame-fields'),normalValue=body.querySelector('#srt-normal-value');
function choose(u){selected=u;selectedBox.innerHTML=`المستخدم المختار: <strong>${esc(u.full_name||u.username||'مستخدم')}</strong>`;grant.disabled=false}
function updateKind(){const isFrame=kind.value==='frame';frameFields.style.display=isFrame?'':'none';normalValue.style.display=isFrame?'none':'';if(kind.value==='verification'){valueLabel.textContent='لون التوثيق';if(!['blue','red','orange'].includes(value.value))value.value='blue'}else if(kind.value==='custom_badge'){valueLabel.textContent='رمز العلامة';value.value='★'}}
kind.onchange=updateKind;updateKind();
body.querySelector('#srt-gift-self').onclick=async()=>{const {data,error}=await client.auth.getUser();if(error||!data?.user){body.querySelector('#srt-gift-status').innerHTML='<div class="srt-status warn">تعذر تحديد حساب الأدمن.</div>';return}const {data:p}=await client.from('profiles').select('id,full_name,username,email,is_verified,verification_color').eq('id',data.user.id).maybeSingle();choose(p||{id:data.user.id,full_name:'حساب الأدمن',username:''})};
search.oninput=()=>{clearTimeout(timer);timer=setTimeout(async()=>{const q=String(search.value||'').trim().replace(/[,%()]/g,' ');if(q.length<2){results.innerHTML='';return}const my=++seq;const{data,error}=await client.from('profiles').select('id,full_name,username,email,is_verified,verification_color').or(`full_name.ilike.%${q}%,username.ilike.%${q}%`).limit(20);if(my!==seq)return;if(error){results.innerHTML=`<div class="srt-status warn">${esc(error.message)}</div>`;return}results.innerHTML=(data||[]).map((u,i)=>`<div class="srt-result" data-index="${i}"><div><strong>${esc(u.full_name||u.username||'مستخدم')}${badge(u)}</strong><div class="srt-meta">@${esc(u.username||'')} · ${esc(u.email||'')}</div></div></div>`).join('');results.querySelectorAll('[data-index]').forEach(el=>el.onclick=()=>{selected=(data||[])[Number(el.dataset.index)];results.querySelectorAll('.srt-result').forEach(x=>x.classList.toggle('active',x===el));choose(selected)})},260)};
grant.onclick=()=>{if(!selected)return;const giftKind=kind.value,label=body.querySelector('#srt-label').value.trim(),color=body.querySelector('#srt-color').value.trim(),frameKey=body.querySelector('#srt-frame-key').value,rawDays=body.querySelector('#srt-frame-days').value,days=rawDays?Math.max(1,Number(rawDays)):null,giftValue=giftKind==='frame'?frameKey:value.value.trim();confirmAction('تأكيد منح الهدية',`سيتم منح ${selected.full_name||selected.username||'المستخدم'} هذه الهدية.`,async()=>{grant.disabled=true;const r=giftKind==='frame'?await client.rpc('student_admin_grant_profile_frame',{p_user:selected.id,p_frame_key:frameKey,p_days:days}):await client.rpc('student_admin_gift_profile_cosmetic',{p_user:selected.id,p_kind:giftKind,p_value:giftValue||null,p_label:label||null,p_color:color||null});grant.disabled=false;if(r.error)throw r.error;body.querySelector('#srt-gift-status').innerHTML='<div class="srt-status ok">تم منح الهدية بنجاح.</div>'})};
}

async function openEducationAudit(){const body=page('student-education-audit-page','فحص اكتمال المراحل');body.innerHTML='<div class="srt-empty">جارٍ فحص هيكل التعليم...</div>';const tables=['education_stages','education_levels','education_subjects','education_tracks','education_level_subjects','education_track_subjects','universities','university_colleges','university_departments','university_levels','university_subjects','university_level_subjects'];const res=await Promise.all(tables.map(t=>client.from(t).select('*',{count:'exact',head:false}).limit(1000)));const errors=res.map((r,i)=>r.error?`${tables[i]}: ${r.error.message}`:null).filter(Boolean);if(errors.length){body.innerHTML=`<div class="srt-status warn">تعذر إكمال الفحص:<br>${errors.map(esc).join('<br>')}</div>`;return}const map=Object.fromEntries(tables.map((t,i)=>[t,res[i].data||[]]));const stages=map.education_stages,levels=map.education_levels,tracks=map.education_tracks,levelLinks=map.education_level_subjects,trackLinks=map.education_track_subjects;const bySlug=Object.fromEntries(stages.map(x=>[x.slug,x]));const issues=[];['primary','middle','secondary','university'].forEach(slug=>{if(!bySlug[slug])issues.push(`المرحلة ${slug} غير موجودة.`)});const expected={primary:6,middle:3,secondary:3};for(const [slug,count] of Object.entries(expected)){const st=bySlug[slug];if(!st)continue;const ls=levels.filter(x=>String(x.stage_id)===String(st.id));if(ls.length<count)issues.push(`${st.name||slug}: الموجود ${ls.length} صفوف والمطلوب الأساسي ${count}.`);for(const l of ls){if(slug==='secondary'){const tr=tracks.filter(x=>String(x.level_id)===String(l.id));if(tr.length<2)issues.push(`${l.name}: يحتاج فرعين أساسيين (علمي/أدبي).`);for(const t of tr)if(!trackLinks.some(x=>String(x.track_id)===String(t.id)))issues.push(`${t.name}: لا توجد مواد مرتبطة.`)}else if(!levelLinks.some(x=>String(x.level_id)===String(l.id)))issues.push(`${l.name}: لا توجد مواد مرتبطة.`)}}if(!map.universities.length)issues.push('لم تتم إضافة جامعات بعد.');const counts={المراحل:stages.length,الصفوف:levels.length,'المواد المدرسية':map.education_subjects.length,'الفروع':tracks.length,'الجامعات':map.universities.length,'الكليات':map.university_colleges.length,'الأقسام':map.university_departments.length,'المراحل الجامعية':map.university_levels.length,'مواد الجامعة':map.university_subjects.length};body.innerHTML=`<div class="srt-grid">${Object.entries(counts).map(([k,v])=>`<div class="srt-metric"><strong>${v}</strong><span>${esc(k)}</span></div>`).join('')}</div><div style="height:10px"></div>${issues.length?`<div class="srt-status warn"><strong>وجدت ${issues.length} نقطة تحتاج مراجعة:</strong><br>${issues.map(x=>`• ${esc(x)}`).join('<br>')}</div>`:'<div class="srt-status ok"><strong>الهيكل التعليمي الأساسي مكتمل.</strong></div>'}<div class="srt-card"><strong>إصلاح الهيكل الأساسي</strong><p class="srt-meta">ينشئ فقط الصفوف الأساسية وفروع العلمي/الأدبي الناقصة. لا يضيف مواد دراسية عشوائية.</p><button class="srt-btn" id="srt-complete-school">إكمال الصفوف والفروع الناقصة</button></div>`;body.querySelector('#srt-complete-school').onclick=()=>confirmAction('إكمال المراحل الأساسية؟','سيتم إنشاء الصفوف والفروع الأساسية المفقودة فقط، ولن تُحذف أو تُعدّل البيانات الحالية.',async()=>{const{error}=await client.rpc('student_admin_complete_school_structure');if(error)throw error;await openEducationAudit()})}

async function init(attempt=0){client=typeof supabaseClient!=='undefined'?supabaseClient:null;if(!client){if(attempt<50)setTimeout(()=>init(attempt+1),250);return}const{data:{user}}=await client.auth.getUser();if(!user)return;const{data}=await client.from('profiles').select('role').eq('id',user.id).maybeSingle();if(String(data?.role||'').toLowerCase()!=='admin')return;adminUser=user;styles();const tools=document.querySelector('.student-admin-tools');if(!tools){if(attempt<50)setTimeout(()=>init(attempt+1),250);return}const defs=[['student-open-recent-users','fa-solid fa-user-clock','المسجلون حديثًا',openRecent],['student-open-profile-gifts','fa-solid fa-gift','هدايا الحسابات',openGifts],['student-open-education-audit','fa-solid fa-list-check','فحص المراحل',openEducationAudit]];defs.forEach(([id,icon,label,fn])=>{if(document.getElementById(id))return;const b=document.createElement('button');b.type='button';b.id=id;b.className='student-admin-tool-button';b.innerHTML=`<i class="${icon}"></i> ${label}`;b.onclick=fn;tools.appendChild(b)})}
