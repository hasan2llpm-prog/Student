...
async function open(){window.StudentNavigation?.prepareRoot?.('messages');if(!await init())return;const p=page();p.classList.add('sm-open');document.body.style.overflow='hidden';S.view='list';S.historyOpen=true;renderListShell();subscribeGlobal();loadConversations({silent:S.conversations.length>0}).catch(console.error)}
